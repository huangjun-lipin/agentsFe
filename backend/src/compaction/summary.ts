...
function formatExistingSummary(messages: BaseMessage[]): string {
    const summaries = messages
        ...
        .map((text) => text.trim())
        .filter(Boolean);
    return summaries.length === 0 ? '无' : summaries.join('\n\n---\n\n');
}
...
export async function generateSummary(
    ...
    try {
        ...
        if (normalizedExistingSummary) return normalizedExistingSummary;
...
export async function compactMessages(
    ...
    const availableForConversation = maxTokens - systemTokens - 500;
    ...
    for (let i = conversationMessages.length - 1; i >= 0; i--) {
        ...
        if (keptTokens + msgTokens <= availableForConversation * 0.6) {
