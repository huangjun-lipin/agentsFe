...
async function main() {
    ...
    const serviceAdapter = new LangChainAdapter({
        chainFn: async ({ messages, tools }) => {
            ...
            if (typeof boundModel === 'function') {
                return (copilotModel as { bindTools: Function }).bindTools(tools).stream(messages);
            }
    ...
    app.listen(PORT, () => {
        ...
        console.log(`[Runtime] CopilotKit endpoint: http://localhost:${PORT}/api/copilotkit`);
        console.log(`[Runtime] Agent endpoint: http://localhost:${PORT}/api/agent/invoke`);
        console.log(`[Runtime] Todos endpoint: http://localhost:${PORT}/api/todos`);
    });
