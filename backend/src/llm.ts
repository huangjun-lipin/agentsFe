...
import type { Config, LLMModelConfig, LLMProvider } from './config/index.js';
...
export function getActiveModelEntry(config: Config): LLMModelConfig {
    ...
    if (!entry) throw new Error(`Active model alias "${alias}" is not configured`);
    return entry;
}
...
export function setActiveModelAlias(config: Config, alias: string): void {
    const entry = config.llm.models.find((item) => item.alias === alias);
    if (!entry) throw new Error(`Model alias "${alias}" not found`);
...
async function loadChatAnthropicCtor(): Promise<new (options: Record<string, unknown>) => BaseChatModel> {
    ...
    try {
        ...
        if (!mod.ChatAnthropic) throw new Error('ChatAnthropic export not found');
...
export async function createChatModel(
    ...
    if (entry.base_url) clientOptions.baseURL = entry.base_url;
    if (defaultHeaders) clientOptions.defaultHeaders = defaultHeaders;
