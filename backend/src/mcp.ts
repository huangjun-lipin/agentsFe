...
import type { Config, MCPConfig, MCPServerConfig } from './config/index.js';
...
function getServerConnectionConfig(serverName: string, server: MCPServerConfig): Record<string, unknown> {
    const effectiveEnv: Record<string, string | undefined> = {
        ...process.env,
        ...(('env' in server && server.env) ? server.env : {}),
    };

    if (server.transport === 'stdio') {
        const command = resolveServerString(server.command, effectiveEnv)?.trim();
        if (!command) throw new Error(`[MCP] Server "${serverName}" is stdio but "command" is empty`);
        return {
            transport: 'stdio',
            command,
    ...
    if (!url) throw new Error(`[MCP] Server "${serverName}" is ${server.transport} but "url" is empty`);
    return {
...
export async function initializeMCPTools(config: Config): Promise<MCPBootstrapResult> {
    if (!config.mcp?.enabled) {
        return { tools: [], close: async () => {}, serverNames: [] };
    }

    const serverNames = Object.keys(config.mcp.servers || {});
    if (serverNames.length === 0) {
        console.warn('[MCP] mcp.enabled=true but no servers configured, skipping.');
        return { tools: [], close: async () => {}, serverNames: [] };
    }
